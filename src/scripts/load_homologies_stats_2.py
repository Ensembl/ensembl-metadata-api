...
with db_conn.session_scope() as session:
    ...
    del_attributes = session.query(Attribute).filter(Attribute.name.startswith('compara.')).delete(synchronize_session='fetch')
    attribute_coverage = Attribute(name=f'compara.homology_coverage ',
                          label=f'compara.homology_coverage ',
                          description=f'compara.homology_coverage ',
                          type='float')
    session.add(attribute_coverage)
    attribute_ref = Attribute(name=f'compara.homology_reference_species ',
                          label=f'compara.homology_reference_species ',
                          description=f'compara.homology_reference_species ',
                          type='string')
    session.add(attribute_ref)
    ...
    for file in files:
        ...
        print(f'Against species in {against}')
        against_organism = session.execute(select(Organism).filter(Organism.ensembl_name == against)).first()[0]
        print(f'Against species in {against_organism}')
        if not homology_datasets:
            ...
            homology_dataset.dataset_attributes.append(DatasetAttribute(attribute=attribute_ref,
                                                                        value=against_organism.scientific_name))
            homology_dataset.dataset_attributes.append(DatasetAttribute(attribute=attribute_coverage,
                                                                        value=str(stat)))
